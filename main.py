...
def main():
    ...
    val_transform = transforms.Compose([  # 强增强
        transforms.Resize((256, 256)),
        transforms.RandomCrop(224),
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    ...
    train_size = int(0.9 * total_labeled)
    val_size = len(full_labeled_dataset) - train_size
    train_labeled_set, val_indices_set = random_split(full_labeled_dataset, [train_size, val_size])

    val_dataset = my_dataset.PersonalityDataset(
        csv_file=args.unlabeled_csv,
        root_dir=args.unlabeled_image_root,
        transform=train_transform,
        is_unlabeled=False
    )
    val_set=torch.utils.data.Subset(val_dataset,val_indices_set.indices)
    # ---------------------------------------------------------
    ...
    for epoch in range(args.epochs):
        ...
        # =====================================================
        # 【方案三】热身完毕后，用成熟的 T2 重新初始化 T1 锚点 7_30_3
        # =====================================================
        #if epoch == warm_up_epochs - 1:
        #    print(f"\n>>> 🔄 热身完毕 (Epoch {epoch+1})，用成熟的 T2 重新初始化 T1 锚点 <<<")
        #    teacher1_anchor.load_state_dict(teacher2_ema.state_dict(), strict=False)
        #    for param in teacher1_anchor.parameters():
...
def adapt_one_epoch(train_loader, student, t1_anchor, t2_ema, optimizer, criterion, criterion_domain, epoch,
    ...
    lambda_style = 0.1
    lambda_ortho = 0.1
    lambda_anchor = 0.5#1.0  # T1 的拉扯力度
...
# C. T1 锚点防守 -> 改为守护源域 (Source) 的拓扑结构！
        ...
        # 7_30_4 反向 rampup：从小到大
        #def get_anchor_loss_weight(epoch, max_weight=0.5, total_epochs=40):
        #    return max_weight * (epoch / total_epochs)  # 线性增长

        #loss_anchor = utils.anchor_feature_consistency_loss(zc_stu, zc_t1_anchor) * get_anchor_loss_weight(epoch)


    # 在 adapt_one_epoch 中增加：#7_18_2
        #z_c_t2 = teacher2_ema(img_teacher, return_features=True)[1]  #7_18_2
        #z_c_t1 = teacher1_anchor(img_teacher, return_features=True)[1]  #7_18_2
        #loss_anchor = utils.anchor_feature_consistency_loss(z_c_t2, z_c_t1) * 0.05  #7_18_2
        ...
        global_step += 1
        if epoch%5==0 and epoch>0:#7_18_1
            anchor_alpha=0.99#7_18_1
            utils.update_ema_variables(student,t1_anchor,anchor_alpha,global_step)#7_18_1
